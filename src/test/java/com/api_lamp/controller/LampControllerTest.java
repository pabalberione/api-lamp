...
public class LampControllerTest {
    ...
    void testCreateLamp() throws Exception{
        Lamp lamp = new Lamp(1L, "Philips",45.26,"Incandescentes",true);
        Mockito.when(lampService.createLamp(any(Lamp.class))).thenReturn(lamp);
    ...
    void testGetLampById() throws Exception{
        Lamp lamp = new Lamp(1L, "Philips",45.26,"Incandescentes",true);
        Mockito.when(lampService.getLampById(anyLong())).thenReturn(Optional.of(lamp));
    ...
    void testGetAllLamps() throws Exception{
        Lamp lamp = new Lamp(1L, "Philips",45.26,"Incandescentes",true);
        Lamp lamp2 = new Lamp(2L, "WIPRO",43.26,"Halogenas",false);

        Mockito.when(lampService.getAllLamps()).thenReturn(Arrays.asList(lamp,lamp2));
    ...
    void testUpdateLamp() throws Exception{
        Lamp lamp = new Lamp(1L, "Philips",45.26,"Updateada",true);
        Mockito.when(lampService.updateLamp(anyLong(), any(Lamp.class))).thenReturn(lamp);
        ...
        mockMvc.perform(MockMvcRequestBuilders.put("/api/v1/lamps/1")
                ...
                .andExpect(jsonPath("$.brand").value("Philips"))
                .andExpect(jsonPath("$.price").value(45.26))
                .andExpect(jsonPath("$.type").value("Updateada"))
                .andExpect(jsonPath("$.available").value("true"));
    }
